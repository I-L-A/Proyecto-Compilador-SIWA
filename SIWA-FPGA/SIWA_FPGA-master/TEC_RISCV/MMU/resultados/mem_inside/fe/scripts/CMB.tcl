...
analyze -library WORK -format sverilog {CMB.sv}

#Luego se analiza los otros módulos
analyze -format sverilog {bus_fsm.sv core_fsm.sv error_handler.sv mem_sgnl_gen.sv fifo.sv controlador.sv}
analyze -format verilog {XSPRAMLP_2048X32_M8P.v}

#Elaboramos el módulo principal
elaborate CMB -architecture verilog -library WORK
...
write -hierarchy -format ddc -output ./db/CMB_unmapped.ddc

#Aplicar especificaciones de diseño (constraints)
source CMB_constraint.tcl
...
#read_saif -input CMB.saif -instance_name tb_CMB/inst_controller -auto_map_names
#Compilar el diseño
compile
...
write -hierarchy -format verilog -output ./db/CMB.v

#Generar los reportes

report_power -analysis_effort high > reports/CMB_power.txt
report_area > reports/CMB_area.txt
report_cell > reports/CMB_cell.txt
report_qor > reports/CMB_qor.txt
report_timing > reports/CMB_timing.txt
report_port > reports/CMB_port.txt

#Escribir el archivo *.ddc (base de datos sintetizada)5
write -hierarchy -format ddc -output ./db/CMB_mapped.ddc
