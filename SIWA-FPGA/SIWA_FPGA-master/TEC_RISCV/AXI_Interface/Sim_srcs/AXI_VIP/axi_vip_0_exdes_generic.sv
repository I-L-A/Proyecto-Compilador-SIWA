...
module axi_vip_0_exdes_generic(
  ...
  //Se crean las variables para el maestro y esclavo adicional
    axi_monitor_transaction                 mst_monitor_transaction2;
    // monitor transaction queue for master VIP 
    axi_monitor_transaction                 master_moniter_transaction_queue2[$];
    // size of master_moniter_transaction_queue
    xil_axi_uint                           master_moniter_transaction_queue_size2 =0;
    //scoreboard transaction from master monitor transaction queue
    axi_monitor_transaction                 mst_scb_transaction2;
    // monitor transaction for slave VIP
    axi_monitor_transaction                 slv_monitor_transaction2;
    // monitor transaction queue for slave VIP
    axi_monitor_transaction                 slave_moniter_transaction_queue2[$];
    // size of slave_moniter_transaction_queue
    xil_axi_uint                            slave_moniter_transaction_queue_size2 =0;
    // scoreboard transaction from slave monitor transaction queue
    axi_monitor_transaction                 slv_scb_transaction2;
  ...
  //Se monitorea el segundo maestro
  initial begin
      #2ps;
      mst_monitor_transaction2 = new("master monitor transaction");
      forever begin
        mst2.agent.monitor.item_collected_port.get(mst_monitor_transaction2);
        if(mst_monitor_transaction2.get_cmd_type() == XIL_AXI_READ) begin
          monitor_rd_data_method_one(mst_monitor_transaction2);
          monitor_rd_data_method_two(mst_monitor_transaction2);
        end  
        master_moniter_transaction_queue2.push_back(mst_monitor_transaction2);
        master_moniter_transaction_queue_size2++;
      end  
    end 
  ...
  //Se monitorea el segundo esclavo
  initial begin
      #2ps;
      slv_monitor_transaction2 = new("slave monitor transaction");
      forever begin
        slv2.agent.monitor.item_collected_port.get(slv_monitor_transaction2);
        slave_moniter_transaction_queue2.push_back(slv_monitor_transaction2);
        slave_moniter_transaction_queue_size2++;
      end
  end
  ...
  //----------------------------------------------------------------------------------------------
  //quedé especificando el orden de 
  integer slv1=0;
  integer slv2=0;
  ...
  initial begin
    forever begin
      wait (slave_moniter_transaction_queue_size>0 ) begin
        ...
        slave_moniter_transaction_queue_size--;
        slv1=slv1+1;
            wait( master_moniter_transaction_queue_size2>0) begin
                mst_scb_transaction2 = master_moniter_transaction_queue2.pop_front;
                master_moniter_transaction_queue_size2--;
                if (slv1>=2 && slv1<5)begin
                    if (slv_scb_transaction.do_compare(mst_scb_transaction2) == 0) begin
                        $display("ERROR: Slave1 VIP against Master2 VIP scoreboard Compare failed");
                        error_cnt++;
                    end else begin
                        $display("SUCCESS: Slave1 VIP against Master2 VIP scoreboard Compare passed");
                    end
                    comparison_cnt++; 
                end
            end           
  ...
  //Comparando las acciones del segundo esclavo
  initial begin
      forever begin
        wait (slave_moniter_transaction_queue_size2>0 ) begin
          slv_scb_transaction2 = slave_moniter_transaction_queue2.pop_front;
          slave_moniter_transaction_queue_size2--;
          slv2=slv2+1;
          wait( master_moniter_transaction_queue_size>0) begin
            mst_scb_transaction = master_moniter_transaction_queue.pop_front;
            master_moniter_transaction_queue_size--;
             if (slv2>=2 && slv2<5)begin
                if (slv_scb_transaction2.do_compare(mst_scb_transaction) == 0) begin
                  $display("ERROR: Slave2 VIP against Master1 VIP scoreboard Compare failed");
                  error_cnt++;
                end else begin
                  $display("SUCCESS: Slave2 VIP against Master1 VIP scoreboard Compare passed");
                end
                comparison_cnt++;
            end
          end  
