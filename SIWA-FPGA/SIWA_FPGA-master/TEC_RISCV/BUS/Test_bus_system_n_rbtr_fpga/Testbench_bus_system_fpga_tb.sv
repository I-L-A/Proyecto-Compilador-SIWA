...
module Sim_bs_systm;
 ...
 bs_gnrtr_n_rbtr_fpga #(`BITS,`DRVRS,`PCKG_SZ,`BROADCAST) uut(
  .clk(clk),
  .reset(reset),
  .pndng(pndng),
  .push(push),
...
//$vcdpluson;
  ...
  task prueba ();
    ...
    for(k=0;k<`BITS;k=k+1)begin
        ...
        begin
          if( push[k][i] == 1 )begin
            $display("At time %t: in terminal %g bus %g message saved. target: %g source: %g ID: %g",$time,i,k, D_push[k][i][`PCKG_SZ-1:`PCKG_SZ-8], D_push[k][i][`PCKG_SZ-9:`PCKG_SZ-16],D_push[k][i][`PCKG_SZ-17:`PCKG_SZ-32]);
    ...
    for(k=0;k<`BITS;k=k+1)begin
        ...
        begin
          if( pop[k][i] == 1 )begin
            $display("At time %t: in terminal %g bus %g message pop. target: %g source: %g ID: %g",$time,i,k, D_pop[k][i][`PCKG_SZ-1:`PCKG_SZ-8], D_pop[k][i][`PCKG_SZ-9:`PCKG_SZ-16],D_pop[k][i][`PCKG_SZ-17:`PCKG_SZ-32]);
