  ...
  task prueba ();
    reset = 0;
      for(i=0;i<`DRVRS;i=i+1)begin
        pndng[i] = {1'b1};
        if(i != `DRVRS-1)begin
          D_pop[i]={target[i+1],source[i],counter[i],{`BITS-32{1'b0}}};
        end else begin
          D_pop[i]={target[0],source[i],counter[i],{`BITS-32{1'b0}}};
        end
    end
        ...
        begin
          if( push[i] == 1 )begin
            $display("At time %t: in terminal %g message saved. target: %g source: %g ID: %g",$time,i, D_push[i][`BITS-1:`BITS-8], D_push[i][`BITS-9:`BITS-16],D_push[i][`BITS-17:`BITS-32]);
        ...
        begin
          if( pop[i] == 1 )begin
            $display("At time %t: in terminal %g message pop. target: %g source: %g ID: %g",$time,i, D_pop[i][`BITS-1:`BITS-8], D_pop[i][`BITS-9:`BITS-16],D_pop[i][`BITS-17:`BITS-32]);
