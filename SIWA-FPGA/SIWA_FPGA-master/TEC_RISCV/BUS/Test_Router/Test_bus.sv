...
//#########################################################
 ...
 //inputs of the UUT: these are the variables to control in the test
  reg clk;
  reg reset;
  reg [`PCKG_SZ-1:0] data_out_i_in[`NUM_NTRFS-1:0];
  reg pndng_i_in[`NUM_NTRFS-1:0];
  reg pop[`NUM_NTRFS-1:0];
 //outputs of the UUT: these are the variables to read in the test
  wire popin[`NUM_NTRFS-1:0];
  wire pndng[`NUM_NTRFS-1:0];
  wire [`PCKG_SZ-1:0] data_out[`NUM_NTRFS-1:0];
 // miscelaneus variables.
  bit [7:0] src;
  bit [3:0] trgt_r;
  bit [3:0] trgt_c;
  bit [`PCKG_SZ-25:0] pyld[3:0];
  bit [7:0] id;

  always_comb begin
    pyld[0] = {(`PCKG_SZ-32){1'b0}};
    pyld[1] = {(`PCKG_SZ-32)/2{2'b01}};
    pyld[2] = {(`PCKG_SZ-32)/2{2'b10}};
    pyld[3] = {(`PCKG_SZ-32){1'b1}};
  end
...
//#########################################################
  ...
  router_bus_gnrtr #(.pck_sz(`PCKG_SZ) ,.num_ntrfs(`NUM_NTRFS), .broadcast(`BROADCAST), .fifo_depth(`FIFO_DEPTH), .id_c(2),.id_r(2)) uut(
  .clk(clk),
  .reset(reset),
...
// This class contain all the mesages to be send in the test, for each interface.
  ...
  class queues #(parameter int num_msgs = 4, parameter int drvrs = 4);
    ...
    function bit push(mnsj mensaje);
      if(index[mensaje.src] < (num_msgs)) begin
        ...
        $display("Messaje pushed to the queues:  from src %g, to trgt_r %g trgt_c %g , with id %g and pyld %h,  at time %g",mensaje.src,mensaje.trgt_r,mensaje.trgt_c,mensaje.id,mensaje.pyld,$time);
        return 1;
      end else begin
        $display("Error there is no space left in the queue for terminal row %g colum %g  at time  %g", mensaje.trgt_r, mensaje.trgt_c,$time);
        $finish;
        return 0;
      end
    endfunction
  ...
  endclass
  queues #(.num_msgs(`NUM_MSGS),.drvrs(`NUM_NTRFS)) input_queues= new;
    ...
    function bit push(mnsj mensaje, int rcvd_in_ntrfs);
      if(index <= (drvrs*num_msgs)) begin // if there is space in the scoreboard 
        for(int i = 0;i < index;i=i+1) begin
          if((enviados[i].src == mensaje.src)&(enviados[i].id==mensaje.id)&(enviados[i].pyld==mensaje.pyld)) begin // if the message match a previous entry
            enviados[i].cmplt = 1;
            enviados[i].tm_rcvd = $time;
            enviados[i].rcvd_in_ntrfs = rcvd_in_ntrfs;
            $display("TRANSACTION COMPLETED: from src %g, to trgt_r: %g trgt_c: %g, with id %g and pyld %h, received in terminal %g, at  at time %g",mensaje.src,mensaje.trgt_r,mensaje.trgt_c,mensaje.id,mensaje.pyld,rcvd_in_ntrfs,$time);
            return 1;
          end
          ...
          index = index+1;
          $display("Message Pushed to the scoreboard: index: %g from src %g, to trgt_r: %g trgt_c: %g, with id %g and pyld %h, received in terminal %g, at  at time %g",index,mensaje.src,mensaje.trgt_r,mensaje.trgt_c,mensaje.id,mensaje.pyld,rcvd_in_ntrfs,$time);
          return 1;
      end else begin // if there is no space left in the scoreboard
        $display("ERROR: There is no space left in the Scoreboard");
        $display("Message from src %g, to trgt_r: %g trgt_c: %g, with id %g and pyld %h, received in terminal %g, at  at time cannot be stored",mensaje.src,mensaje.trgt_r,mensaje.trgt_c,mensaje.id,mensaje.pyld,rcvd_in_ntrfs,$time);
        $finish;
        test_status = 0;
        return 0;
      end
