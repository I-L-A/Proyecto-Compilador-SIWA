...
module router_bus_gnrtr #(parameter pck_sz = 40, parameter num_ntrfs=4, parameter broadcast = {8{1'b1}}, parameter fifo_depth=4,parameter id_c = 0,parameter id_r = 0)(
  input clk,
  input reset,
  ...
  generate
    for(i=0;i<num_ntrfs; i=i+1)begin:_nu_
      router_bus_interface #(.pck_sz(pck_sz),.num_ntrfs(num_ntrfs), .ntrfs_id(i),.broadcast(broadcast),.fifo_depth(fifo_depth),.id_c(id_c),.id_r(id_r)) rtr_ntrfs_ (
        .clk(clk),
        .reset(reset),
        .data_in_i(data_in_i),
        .trn(trn),
...
module router_bus_interface #(parameter pck_sz = 40, parameter num_ntrfs=4, parameter [7:0]ntrfs_id = 0, parameter broadcast = {8{1'b1}}, parameter fifo_depth=4, parameter id_c=0,parameter id_r=0) (
  input clk,
  input reset,
  ...
  s_routing_table #(.id_r(id_r),.id_c(id_c),.pckg_sz(pck_sz)) rt (
...
`ifdef DEBUG
  always@(posedge psh) begin
    $display("Message received in terminal: %g router ID: %g %g", ntrfs_id,id_r,id_c );
...
`endif
  ...
  assign data_out_i = data_out_i_in;
...
module rtr_rbtr_cntrl (
 ...
 always_comb begin
   case(cur_st)
     rst_st: begin
       nxt_st <= pndng?psh1_st:rst_st;
     end 
     psh1_st: nxt_st <= pop1_st;
     pop1_st: nxt_st <= rst_st;
     default: nxt_st <= rst;
  endcase
 end
...
module s_routing_table #(parameter id_r =0, parameter id_c = 0, parameter pckg_sz =32) (
  input [pckg_sz-1:0] Data_in,
  output logic [pckg_sz-1:0] Data_out_i
);
  always_comb begin
    Data_out_i[pckg_sz-9:0] = Data_in[pckg_sz-9:0];
    if(Data_in[pckg_sz-17]) begin
      if(Data_in[pckg_sz-9:pckg_sz-12] < id_r) begin
        Data_out_i[pckg_sz-1:pckg_sz-8] <= 2;
      end
      if(Data_in[pckg_sz-9: pckg_sz-12] > id_r) begin
        Data_out_i[pckg_sz-1:pckg_sz-8] <= 0;
      end
      if(Data_in[pckg_sz-9: pckg_sz-12] == id_r) begin
        if(Data_in[pckg_sz-13: pckg_sz-16] < id_c) begin
          Data_out_i[pckg_sz-1:pckg_sz-8] <= 3;
        end
        if(Data_in[pckg_sz-13: pckg_sz-16] > id_c) begin
          Data_out_i[pckg_sz-1:pckg_sz-8] <= 1;
        end
        if(Data_in[pckg_sz-13: pckg_sz-16]== id_c) begin
          Data_out_i[pckg_sz-1:pckg_sz-8] <= 4;
        end
      end
    end else begin
      if(Data_in[pckg_sz-13: pckg_sz-16] < id_c) begin
      ...
      if(Data_in[pckg_sz-9: pckg_sz-12] == id_r) begin
        ...
        if(Data_in[pckg_sz-13: pckg_sz-16] > id_r) begin
          Data_out_i[pckg_sz-1:pckg_sz-8] <= 2;
        end
        if(Data_in[pckg_sz-13: pckg_sz-16] == id_r) begin
          Data_out_i[pckg_sz-1:pckg_sz-8] <= 4;
      ...
      for(C=1;C <= COLUMS; C=C+1)begin:_clm_
        ...
        if(R == 1) begin
          if(C == 1) begin //upper left corner
              ...
              conector #(.size(1)) buf_pop_1 (.out(pop_connected[1]),.in(pop_hor_3_1[R-1][C-1]));
              ...
              conector #(.size(1)) buf_pop_2 (.out(pop_connected[2]),.in(pop_ver_0_2[R-1][C-1]));
          ...
          if(C ==COLUMS) begin //upper rigth corner
              ...
              conector #(.size(1)) buf_pop_3 (.out(pop_connected[3] ),.in(pop_hor_1_3[R-1][C-2]));
        ...
        if(R == ROWS) begin
          if(C == 1) begin //bottom left corner
              ...
              conector #(.size(1)) buf_pop_0 (.out(pop_connected[0] ),.in(pop_ver_2_0[R-2][C-1]));
              ...
              conector #(.size(1)) buf_pop_2 (.out(pop_connected[0] ),.in(pop[COLUMS+ROWS]));
          ...
          if(C == COLUMS) begin //bottom rigth corner
              ...
              conector #(.size(1)) buf_pop_2 (.out(pop_connected[0] ),.in(pop[2*COLUMS+ROWS-1]));
          ...
          if((C != COLUMS)&&(C != 1)) begin //Last row (not corners)
              ...
              conector #(.size(1)) buf_pop_2 (.out(pop_connected[0] ),.in(pop[COLUMS+ROWS-1+C]));
        ...
        if((C != COLUMS)&&(C != 1)&&(R != 1)&&(R != ROWS))begin //Every router not connected to an agent
          ...
          router_bus_gnrtr #(.pck_sz(pckg_sz),.num_ntrfs(4),.broadcast(bdcst),.fifo_depth(fifo_depth),.id_c(C),.id_r(R)) rtr(
            .clk(clk),
            .reset(reset),
