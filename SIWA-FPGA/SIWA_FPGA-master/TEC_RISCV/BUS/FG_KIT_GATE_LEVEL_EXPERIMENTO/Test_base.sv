...
//#########################################################
 //inputs
  ...
  reg [`BITS-1:0][`DRVRS-1:0]pndng;
  //reg [`PCKG_SZ-1:0] D_pop[`BITS-1:0][`DRVRS-1:0];
  reg [`BITS-1:0][`DRVRS-1:0][`PCKG_SZ-1:0]D_pop;
 //outputs
  //wire push[`BITS-1:0][`DRVRS-1:0];
  wire [`BITS-1:0][`DRVRS-1:0]push;
  //wire pop[`BITS-1:0][`DRVRS-1:0];
  wire [`BITS-1:0][`DRVRS-1:0]pop;
  //wire [`PCKG_SZ-1:0] D_push[`BITS-1:0][`DRVRS-1:0];
  wire [`BITS-1:0][`DRVRS-1:0][`PCKG_SZ-1:0]D_push;
...
//#########################################################
  ...
  //*******************************************
  class vector_mensajes #(parameter int bits = 1, parameter int drvrs = 4,parameter int num_msgs = 4);
    protected mnsj enviados[bits*drvrs*num_msgs];
    protected int index = 0;
    protected int completed = 0;
    protected bit test_status[bits] = '{bits{1}};

    function bit push(mnsj mensaje);
      if(index < (bits*drvrs*num_msgs)) begin
  ...
  vector_mensajes #(.bits(`BITS),.drvrs(`DRVRS),.num_msgs(`NUM_MSGS*2)) mensajes_enviados = new;
...
//#########################################################
  ...
  task prueba ();
      reset = 0;
//    force Sim_bs_systm.uut.BUS[0].ID[0].ntrfs.D_push[20] = 0;
    //************************************************************
