...
//    end
  ...
  begin
     ...
     cnt_clk_en <= (bs_grnt == 0 )?{1'b1}:{1'b0}; 
     cntr_clk <= clk && cnt_clk_en && rqst_xst;
  end
endmodule
...
module bs_gnrtr #(parameter drvrs = 4, parameter pckg_sz = 16, parameter broadcast = {8{1'b1}}) (
  ...
  Arbiter#(drvrs) arb_inst (.reset(reset),.clk(clk),.bs_rqst(bs_rqst),.bs_grnt(bs_grnt));
endmodule
...
module prll_intrfs_cntrl #(parameter id = 0, parameter bdcst = 255, parameter drvrs = 4) (
 ...
 prll_rd_st_mchn rd_st_mchn(.c_r(c_r),.reset(reset),.clk(clk),.rst_r(rst_r),.push(push),.rdi(rdi),.s_ds_r(s_ds_r));
   tri_buf trn_chng_buf(.a(trn_chng_pre),.b(trn_chng),.en(bs_grnt));
   tri_buf wrt_buf(.a(wrt_pre),.b(wrt),.en(bs_grnt));
...
module prll_bs_gnrtr_n_rbtr #(parameter bits = 1,parameter drvrs = 4, parameter broadcast = {8{1'b1}}) (
  input clk,
  input reset,
  input  pndng[drvrs-1:0],
  output push[drvrs-1:0],
  output pop[drvrs-1:0],
  input  [bits-1:0] D_pop[drvrs-1:0],
  output [bits-1:0] D_push[drvrs-1:0]
);
  wire [bits-1:0] bus;
  wire trn_chng;
  wire wrt;
...
// Definition of the prll bus system
///////////////////////////////////////////////////////////////////////
module fifo #(parameter depth = 16,parameter bits = 32)(
  input [bits-1:0] Din,
  ...
  always@(posedge push or posedge rst or posedge pop)begin
  if(rst) begin
    count <= 0;
  end else begin
