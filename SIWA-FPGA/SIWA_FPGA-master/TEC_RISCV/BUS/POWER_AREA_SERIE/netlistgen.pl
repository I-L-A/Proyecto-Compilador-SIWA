...
#general variables:
my $archivo_comandos = $ARGV[0];
my $netlist_base = $ARGV[1];
my $model_netlist = "";
my %comandos = ();
...
while (my $row = <$fh1>){
	  chomp $row;
	   my @lista_linea_comandos = split /;/,$row;
	   $comandos{$lista_linea_comandos[0]} =$lista_linea_comandos[1];
...
#      print "test: ${netlist_inst}_${nemo}_${nemo_val}\n";
    }  
  }
