...
module RF_tb;
	...
	reg [3:0] csr_addr;
	reg  reg_w_data;
	reg w_enable, clk, csr_rd_en, csr_wr_en, reset;
	reg load;
	reg [95:0] csr_trap;
	reg [29:0] pc;
	reg mip_IO;
	...
	task registerRead (reg [4:0] rs_1, rs_2, wire int outR1, outR2);
		#10 r_enable = 1;
		rs1 = rs_1;
		rs2 = rs_2;
	...
	task csrRead (reg bit [4:0] rs_1, rs_2, csr_address, wire int outCSR);
		#10 csr_rd_en = 1;
		rs1 = rs_1;
		rs2 = rs_2;
	...
	task registerWrite (reg bit [4:0] reg_dir, reg bit  write_value);
		w_enable = 1;
		rd = reg_dir;
	...
	task csrWrite (reg bit [4:0] csr_dir, reg bit  write_value);
		csr_wr_en = 1;
		csr_addr = csr_dir;
		reg_w_data = write_value;
		#25 csr_wr_en = 0;
