...
# ========= Reglas de análisis (mismas que tu script) =========
...
def analizar_codigo(path_c: str) -> list[str]:
    ...
    for num, linea in enumerate(lineas, 1):
        ...
        if re.search(r"\bcsrr\w*\b", linea):
            ...
            if match and int(match.group(), 16) > 0x07:
...
class App(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("SIWA Builder GUI — TEC-RISC-V")
        ...
        self.addr_text = tk.StringVar(value="0x00001000")  # Dirección de texto si no hay linker.ld
    ...
    def _build_ui(self):
        ...
        tk.Entry(self, textvariable=self.path_c, width=70).grid(row=0, column=1, **pad)
        ...
        tk.Entry(self, textvariable=self.path_linker, width=70).grid(row=1, column=1, **pad)
        tk.Button(self, text="Seleccionar…", command=self.select_linker).grid(row=1, column=2, **pad)

        # Toolchain y opciones
        tk.Label(self, text="Toolchain prefix:").grid(row=2, column=0, sticky="e", **pad)
        tk.Entry(self, textvariable=self.tool_prefix, width=18).grid(row=2, column=1, sticky="w", **pad)

        tk.Label(self, text="-march").grid(row=2, column=1, sticky="e", padx=(0,140))
        tk.Entry(self, textvariable=self.march, width=10).grid(row=2, column=1, sticky="e", padx=(0,80))

        tk.Label(self, text="-mabi").grid(row=2, column=1, sticky="e", padx=(0,40))
        tk.Entry(self, textvariable=self.mabi, width=10).grid(row=2, column=1, sticky="e", padx=(0,0))

        tk.Label(self, text="Dirección texto si no hay linker (hex):").grid(row=3, column=0, sticky="e", **pad)
        tk.Entry(self, textvariable=self.addr_text, width=18).grid(row=3, column=1, sticky="w", **pad)
    ...
    def on_build(self):
        ...
        if self.want_elf.get():
            # Si hay linker.ld úsalo, si no, fija una dirección de texto razonable
            if linker:
                cmd = [gcc, f"-march={march}", f"-mabi={mabi}", "-nostdlib", "-T", linker,
                       "-o", f"{base}.elf", f"{base}.s" if os.path.exists(f"{base}.s") else src]
            else:
                # Fallback sin linker: ubicar .text en 0x00001000 (ajústalo a tu mapa SIWA)
                cmd = [gcc, f"-march={march}", f"-mabi={mabi}", "-nostdlib",
                       f"-Wl,-Ttext={addr_text}", "-o", f"{base}.elf",
    ...
    def on_qemu(self):
        ...
        cmd = [qemu, "-machine", "sifive_e", "-bios", "none", "-kernel", elf, "-nographic", "-serial", "mon:stdio"]
        self._log("🚀 Ejecutando en QEMU (Ctrl+A luego X para salir)…\n")
        # Abrimos QEMU en terminal externa para no congelar la GUI
        if sys.platform.startswith("win"):
